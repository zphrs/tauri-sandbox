...
class FetchEvent extends Event {
    ...
    get respondWith() {
        ...
        return function (resp) {
            if (__privateGet(t, _customRespondWith))
                __privateGet(t, _customRespondWith).bind(this, resp)()
            __privateMethod(t, _FetchEvent_instances, respondWith_fn).bind(
                this,
                resp,
            )()
        }
...
async function overrideLocalStorage(docId) {
    ...
    const { port, initialStore } = await new Promise((res) => {
        window.addEventListener("message", (ev) => {
    ...
    if (iframe.contentWindow) {
        ...
        iframe.addEventListener("load", () => {
            ...
            ;(_a = iframe.contentWindow) == null
                ? void 0
                : _a.postMessage("localStorageInit", "*", [childPort])
        })
    }

...
describe("abort-in-initial-upgradeneeded", () => {
    test("An abort() in the initial onupgradeneeded sets version back to 0", async ({
        task,
    }) => {
        const dbname =
        ...
        const upgradeAndAbortPromise = new Promise<void>((resolve, reject) => {
            open_rq.onupgradeneeded = function (e) {
                db = (e.target as IDBOpenDBRequest).result
                expect(db.version).toBe(2)

                transaction = (e.target as IDBOpenDBRequest).transaction!
        ...
        const errorPromise = new Promise<void>((resolve) => {
            open_rq.onerror = function (e) {
                ...
                expect((e.target as IDBOpenDBRequest).result).toBeUndefined()

...
// Port of w3c test: idbindex_getAll-options.tentative.any.js
// Tests IDBIndex.getAll() with options dictionary
...
// Main test function for index getAll with options
async function indexGetAllValuesWithOptionsTest(
    task: { id?: string },
    storeName: string,
    options: {
        query?: IDBValidKey | IDBKeyRange
    ...
    // Simulate the options dictionary by using the existing getAll API
    let request: IDBRequest
    if (options.count) {
        request = queryTarget.getAll(options.query, options.count)
    } else if (options.query) {
        request = queryTarget.getAll(options.query)
    } else {
...
// Test for getAll with both options and count
async function getAllWithOptionsAndCountTest(
    task: { id?: string },
    storeName: string,
): Promise<void> {
    const { db, expectedRecords } = await setupIndex(task, storeName)
    ...
    const request = queryTarget.getAll(undefined, options.count)
    const actualResults = await requestToPromise(request)
    const expectedResults = calculateExpectedGetAllResults(
        expectedRecords,
        options,
...
describe("IDBIndex.getAll() with options dictionary", () => {
    test("Single item get", async ({ task }) => {
        await indexGetAllValuesWithOptionsTest(task, "out-of-line", {
            query: "C",
        })
    })

    test("Empty object store", async ({ task }) => {
        await indexGetAllValuesWithOptionsTest(task, "empty", {})
    })
    ...
    test("Get all with generated keys", async ({ task }) => {
        await indexGetAllValuesWithOptionsTest(task, "generated", {})
    })

    test("Get all with large values", async ({ task }) => {
        await indexGetAllValuesWithOptionsTest(task, "large-values", {})
    })
    ...
    test("Retrieve multiEntry key", async ({ task }) => {
        await indexGetAllValuesWithOptionsTest(task, "out-of-line-not-unique", {
            query: "first",
        })
    })
    ...
    test("Get all values with both options and count", async ({ task }) => {
        await getAllWithOptionsAndCountTest(task, "out-of-line")
    })

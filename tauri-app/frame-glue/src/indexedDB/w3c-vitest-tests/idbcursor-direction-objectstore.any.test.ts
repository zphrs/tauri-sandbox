...
// Port of w3c test: idbcursor-direction-objectstore.any.js
// Tests IDBCursor direction - object store
...
describe("IDBCursor direction - object store", () => {
    cases.forEach((testcase) => {
        const { dir, expect: expectedValues } = testcase

        test(`IDBCursor direction - object store - ${dir}`, async ({
            task,
        }) => {
            ...
            const request = objStore.openCursor(undefined, dir)

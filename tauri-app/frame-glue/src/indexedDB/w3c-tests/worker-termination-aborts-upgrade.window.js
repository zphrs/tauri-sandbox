...
promise_test(async (t) => {
    ...
    t.add_cleanup(() => {
        indexedDB.deleteDatabase("aborted-upgrade-db")
    })
    let createdDB = await migrateNamedDatabase(
        t,
        "aborted-upgrade-db",
        1,
        (db) => {
    ...
    assert_equals(
        createdDB.objectStoreNames.length,
        1,
        "created object store correctly",
    )

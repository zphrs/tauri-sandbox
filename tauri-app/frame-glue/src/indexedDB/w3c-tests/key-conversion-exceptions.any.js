...
;["add", "put"].forEach((method) => {
    indexeddb_upgrade_only_test((t, db) => {
        ...
        const in_line = db.createObjectStore("in-line keys", {
            keyPath: "prop",
        })
...
;["delete", "get", "getKey", "count", "openCursor", "openKeyCursor"].forEach(
    (method) => {
        indexeddb_upgrade_only_test((t, db) => {
            const store = db.createObjectStore("store")
...
;["get", "getKey", "count", "openCursor", "openKeyCursor"].forEach((method) => {
    indexeddb_upgrade_only_test((t, db) => {
        const store = db.createObjectStore("store")

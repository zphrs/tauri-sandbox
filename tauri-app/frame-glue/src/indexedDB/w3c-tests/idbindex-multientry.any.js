...
async_test((t) => {
    ...
    for (let i = 0; i < 1000; i++) obj.idxkeys.push("index_no_" + i)
    open_rq.onupgradeneeded = function (e) {
        db = e.target.result
        db.createObjectStore("store").createIndex("index", "idxkeys", {
            multiEntry: true,
        })
    }
    open_rq.onsuccess = function (e) {
        let tx = db.transaction("store", "readwrite")
        tx.objectStore("store").put(obj, 1).onsuccess = t.step_func(
            function (e) {

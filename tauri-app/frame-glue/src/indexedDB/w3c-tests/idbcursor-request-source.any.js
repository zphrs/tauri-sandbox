...
function cursor_request_source_test(
    ...
    indexeddb_test(
        ...
        (t, db) => {
            ...
            openCursorRequest.onerror = t.unreached_func(
                "The cursor must not fail to open.",
            )

            openCursorRequest.onsuccess = t.step_func((e) => {
                // Use the cursor to create a new request.
                const cursor = e.target.result
                const request = cursor[createRequestFunctionName](
                    ...createRequestFunctionArgs,
                )
                assert_equals(
                    request.source,
...
function open_cursor_request_source_test(
    ...
    indexeddb_test(
        ...
        (t, db) => {
            ...
            openCursorRequest.onerror = t.unreached_func(
                "The cursor must not fail to open or iterate.",
            )

            assert_equals(
                openCursorRequest.source,
                cursorSource,
        ...
        `${cursorSourceType}::${
            openCursorFunctionName
        }'s request source must be the ${
            cursorSourceType
        } instance that opened the cursor`,
    )
}

...
async_test((t) => {
    ...
    open_rq.onupgradeneeded = function (e) {
        ...
        let index = objStore.createIndex("index", "indexedProperty", {
            unique: true,
        })
...
async_test((t) => {
    ...
    open_rq.onupgradeneeded = function (e) {
        ...
        e.target.transaction.oncomplete = fail(
            t,
            "got complete, expected abort",
        )
    }
...
async_test((t) => {
    ...
    open_rq.onupgradeneeded = function (e) {
        ...
        for (let i = 0; i < 5; i++) objStore.add("object_" + i, i)

        let rq = objStore.createIndex("index", "")
        rq.onerror = function () {
            assert_unreached("error: " + rq.error.name)
        }
        rq.onsuccess = function () {}

        objStore.index("index").get("object_4").onsuccess = t.step_func(
            function (e) {
...
async_test((t) => {
    ...
    open_rq.onupgradeneeded = function (e) {
        ...
        for (let i = 0; i < 5; i++) store.add({ idx: "object_" + i }, i)
...
indexeddb_test(
    ...
    function (t, db) {
        ...
        store.openCursor().onsuccess = t.step_func(function (e) {
            ...
            store.index("CompoundKey").get([item.num, item.id]).onsuccess =
                t.step_func(function (e) {
                    assert_equals(
                        e.target.result ? e.target.result.num : null,
                        100,
                        "Expected 100.",
                    )
                    t.done()
...
indexeddb_test(
    function (t, db, txn) {
        // Auto-increment
        let store = db.createObjectStore("Store2", {
            keyPath: "id",
            autoIncrement: true,
        })
        store.createIndex("CompoundKey", ["num", "id"])
...
indexeddb_test(
    function (t, db, txn) {
        // Auto-increment
        let store = db.createObjectStore("Store3", {
            keyPath: "id",
            autoIncrement: true,
        })
    ...
    function (t, db) {
        ...
        const keys = []
        let count
        store.count().onsuccess = t.step_func((e) => {
        ...
        store.index("CompoundKey").openCursor().onsuccess = t.step_func(
            function (e) {
                ...
                assert_equals(
                    keys.length,
                    2,
                    "Expected exactly two index entries.",
                )

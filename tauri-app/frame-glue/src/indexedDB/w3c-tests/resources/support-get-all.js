...
function get_all_test(
    ...
    const testGetAllCallback = (test, connection, expectedRecords) => {
        ...
        request.onerror = test.unreached_func(
            "The get all request must succeed",
        )
...
function object_store_get_all_keys_test(storeName, options, testDescription) {
    get_all_test(
        ...
        /*indexName=*/ undefined,
        options,
        /*shouldUseDictionaryArgument=*/ false,
        testDescription,
    )
}
...
function object_store_get_all_values_with_options_test(
    ...
    get_all_test(
        ...
        /*indexName=*/ undefined,
        options,
        /*shouldUseDictionaryArgument=*/ true,
        testDescription,
    )
}
...
function calculateExpectedGetAllResults(getAllFunctionName, records, options) {
    ...
    switch (getAllFunctionName) {
        case "getAll":
            return expectedRecords.map(({ value }) => {
                return value
            })
        case "getAllKeys":
            return expectedRecords.map(({ primaryKey }) => {
                return primaryKey
            })
        case "getAllRecords":
        ...
        for (let property_name of Object.keys(expected_value)) {
            if (isArrayOrArrayBufferView(expected_value[property_name])) {
                ...
                assert_large_array_equals(
                    ...
                    `The record must contain the array value "${JSON.stringify(
                        expected_value,
                ...
                assert_equals(
                    actual_value[property_name],
                    expected_value[property_name],
                    `The record must contain the value "${JSON.stringify(
                        expected_value,
                    )}" with property "${property_name}"`,
                )
            }
...
function assert_record_equals(actual_record, expected_record) {
    assert_class_string(
        actual_record,
        "IDBRecord",
        "The record must be an IDBRecord",
    )
    assert_idl_attribute(
        actual_record,
        "key",
        "The record must have a key attribute",
    )
    assert_idl_attribute(
        actual_record,
        "primaryKey",
...
function assert_records_equals(actual_records, expected_records) {
    ...
    assert_equals(
        actual_records.length,
        expected_records.length,
...
function get_all_with_options_and_count_test(
    ...
    const setupFunction = optionalIndexName
        ? index_get_all_test_setup
        : object_store_get_all_test_setup

    setupFunction(
        storeName,
        (test, connection, expectedRecords) => {
            const transaction = connection.transaction(storeName, "readonly")
            let queryTarget = transaction.objectStore(storeName)
            if (optionalIndexName) {
                queryTarget = queryTarget.index(optionalIndexName)
            ...
            const request = queryTarget[getAllFunctionName](
                options,
                /*count=*/ 17,
            )

            request.onerror = test.unreached_func(
                `"${getAllFunctionName}()" request must succeed.`,
            )
    ...
    setupFunction(
        ...
        (test, connection, expectedRecords) => {
            ...
            invalidKeys.forEach(({ description, value }) => {
                const argument = shouldUseDictionaryArgument
                    ? { query: value }
                    : value
                assert_throws_dom(
                    "DataError",

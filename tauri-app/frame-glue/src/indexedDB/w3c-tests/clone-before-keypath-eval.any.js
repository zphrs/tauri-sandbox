...
indexeddb_test(
    createObjectStoreWithKeyPath("store", "id", true),
    (t, db) => {
...
indexeddb_test(
    createObjectStoreWithKeyPath("store", "invalid_id", true),
    (t, db) => {
        const { store } = createTransactionAndReturnObjectStore(db, "store")
        const obj = new ProbeObject()
        assert_throws_dom(
...
indexeddb_test(
    createObjectStoreWithIndex("store", null, "index", "prop"),
    (t, db) => {
        const { store } = createTransactionAndReturnObjectStore(db, "store")
        const obj = new ProbeObject()
        store.put(obj, "key")
...
indexeddb_test(
    createObjectStoreWithIndex("store", "id", "index", "prop"),
    (t, db) => {
        const { store } = createTransactionAndReturnObjectStore(db, "store")
        const obj = new ProbeObject()
        store.put(obj)
